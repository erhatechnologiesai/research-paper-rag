...
An academic paper analysis assistant designed to ingest scientific PDFs, parse methodology, abstract, and results sections, and generate comparative literature summaries with BibTeX citations.

---

## Key Features

- **Section-aware**: parsing segmenting Papers into Abstract, Methodology, Results, and Discussion
- **BibTeX**: metadata extraction and formatted citation generation
- **Comparative**: QA identifying conflicting findings across research papers
- **Mathematical**: notation and table structure preservation
- **FastAPI**: endpoint for structured literature analysis
...
```mermaid
flowchart LR
    Paper[Scientific Paper PDF] --> Parser[Section-Aware Parser]
    Parser --> Sections[(Abstract, Method, Results)]
    Query([Literature Query]) --> RAG[Scientific RAG Engine]
    Sections --> RAG
    RAG --> Output[Synthesized Analysis + BibTeX Citation]
```
...
├── tests/
│   ├── __init__.py
│   └── test_paper_rag.py   # Automated test suite
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/erhatechnologiesai/research-paper-rag.git
...
|---|---|---|
| `POST` | `/analyze` | Analyze paper content or compare findings across papers |

### Example Request

```bash
curl -X POST http://127.0.0.1:8000/analyze -H "Content-Type: application/json" -d '{"paper_text": "Transformer models utilize self-attention mechanisms...", "query": "What is the primary contribution?"}'
```
...
```bash
python -m unittest tests/test_paper_rag.py
```
